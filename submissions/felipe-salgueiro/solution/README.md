...
pipeline completo Bronze/Prata/Ouro possuem scripts determinísticos versionados.
...
**Limites conhecidos:** `creator_profile_eligible` foi gerado por regra superada. O valor `measure_better` é fixo na implementação histórica do Ouro, não saída de um classificador de decisões validado. Não consumir esses campos como política do produto. Consulte a [revisão oficial](../docs/05-validacao/checkpoint-revisao-dicionario-oficial.md); o contrato/exportador Ouro para a UI ainda está pendente.
